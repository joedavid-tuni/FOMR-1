...
I=find(dist(:,1)==v);
if ~isempty(I)
    % if it is true then return its cost
    dst= dist(I,2);
else   
    ...
    I=size(dist,1);
    for i=1:length(u)    
        % find minimum distances way from next connected node
        [dst2 dist parent] =sp_dp(graph,u(i),dist,parent,Loc,Ind,deep); 
        % calculate begin to current plus next to end cost
        new_distance=costcal(Loc(Ind(u(i)),:),Loc(Ind(v),:))+ dst2;
        % if this cost is less than before then update
